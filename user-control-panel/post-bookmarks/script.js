    ...
    if ($(this).hasClass('opacity')) {
        ...
        if (unbookmark) {
            ...
            $.get('/index.php?act=UserCP&CODE=01', function(data) {
                ...
                $.post('/index.php?auth_key=' + pMarkAuthKey, $('form[name="theForm"]', data).serialize() + '&' + $('td.pformleft:contains(Bookmarks)', data).next('td.pformright').find('textarea').attr('name') + '=' + replace, function() {
                    $this.css('opacity', '0.5');
                });
            });
        }
        ...
        $.get('/index.php?act=UserCP&CODE=01', function(data) {
            $.post('/index.php?auth_key=' + pMarkAuthKey, $('form[name="theForm"]', data).serialize() + '&' + $('td.pformleft:contains(Bookmarks)', data).next('td.pformright').find('textarea').attr('name') + '=' + $('td.pformleft:contains(Bookmarks)', data).next('td.pformright').find('textarea').val() + '\n' + fID + '~' + tID + '~' + pID + '~' + label + '~' + message + '~' + tTitle + '~' + pAuthor + '~' + month + '/' + date + '/' + year, function() {
                $this.css('opacity', '1');
            });
        });
    }
