...
$(function() {
    ...
    $('div.row2[id*="field"]').each(function() {
        for (var x = 0; x < countryN.length; x++) {
            if (countryN[x] === $(this).text().split('Country: ')[1]) {
                $(this).html('Country: <img src="' + countryI[x] + '" alt="' + countryN[x] + '" title="' + countryN[x] + '" />');
            }
        }
    });

    $('span.postdetails').each(function() {
        for (var x = 0; x < countryN.length; x++) {
            $(this).html($(this).html().replace('Country: ' + countryN[x], 'Country: <img src="' + countryI[x] + '" alt="' + countryN[x] + '" />'));
        }
    });
